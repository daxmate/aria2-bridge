...
- **签名 URL 自动放行** — 下载 URL 带一次性签名 token（如 ankiweb 的 `?t=eyJ...`）、阿里云 OSS 签名直链（`OSSAccessKeyId`/`Signature`），或来自已知签名直链站点（内置 `jiaoyanyun.com` / `speiyou.com` 域名，按 URL/Referrer 判断）时，直接走浏览器原生下载：此类签名 URL 在浏览器请求时即被消耗，或 CDN 会按请求特征返回不同内容（实测 aria2 下载拿到过损坏文件），可在设置页关闭
...
**特殊处理：签名 URL（JWT token / OSS 直链 / 已知站点）** — 部分网站的下载 URL 带一次性签名 token（如 ankiweb 的 `?t=eyJ...`）、阿里云 OSS 签名直链（`OSSAccessKeyId`/`Signature`），或属于内置签名直链站点（`jiaoyanyun.com` / `speiyou.com`，按 URL 或 Referrer 域名判断——这些站点的下载 URL 形态多样，不一定带签名参数）。浏览器发起请求时 token 即被消耗，或 CDN 会按请求特征（UA/Referer/Cookie）返回不同内容（实测 jiaoyanyun 的 CDN 对 aria2 请求返回过损坏的旧缓存文件，也出现过浏览器下载被取消、aria2 却拿不到文件的“点击落空”）。这类下载直接放行，由浏览器原生完成。
...
| **跳过签名 URL 下载** | 开启后，URL 带一次性签名 token（如 ankiweb `?t=eyJ...`）、阿里云 OSS 签名直链（`OSSAccessKeyId`/`Signature`），或命中内置签名直链站点（`jiaoyanyun.com` / `speiyou.com`，按 URL/Referrer 域名判断）的下载不转发 aria2，直接浏览器原生下载。默认开启 |
...
- 带一次性签名 token、OSS 签名直链或命中内置签名直链站点（如 ankiweb、jiaoyanyun 的 jiaoyanyun.com/speiyou.com 域名）的下载直接走浏览器原生下载，不会进入 aria2——此类签名 URL 在浏览器请求时即被消耗，或 CDN 会按请求特征返回不同内容（实测 aria2 拿到过损坏文件，也出现过点击落空误判失败）。如确需转发，可在设置页关闭「跳过签名 URL 下载」
