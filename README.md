...
- **签名 URL 自动放行** — URL 带一次性签名 token（如 ankiweb 的 `?t=eyJ...`）或阿里云 OSS 签名直链（`OSSAccessKeyId`/`Signature`，如教育云资源站）的下载直接走浏览器原生下载：此类签名 URL 在浏览器请求时即被消耗，或 CDN 会按请求特征返回不同内容（实测 aria2 下载拿到过损坏文件），可在设置页关闭
...
**特殊处理：签名 URL（JWT token / OSS 直链）** — 部分网站的下载 URL 带一次性签名 token（如 ankiweb 的 `?t=eyJ...`）或阿里云 OSS 签名直链（`OSSAccessKeyId`/`Signature`）。浏览器发起请求时 token 即被消耗，或 CDN 会按请求特征（UA/Referer/Cookie）返回不同内容（实测 jiaoyanyun 的 CDN 对 aria2 请求返回了损坏的旧缓存文件）。这类下载直接放行，由浏览器原生完成。
...
| **跳过签名 URL 下载** | 开启后，URL 带一次性签名 token（如 ankiweb `?t=eyJ...`）或阿里云 OSS 签名直链（`OSSAccessKeyId`/`Signature`）的下载不转发 aria2，直接浏览器原生下载。默认开启 |
...
- 带一次性签名 token 或 OSS 签名直链的下载（如 ankiweb、jiaoyanyun）直接走浏览器原生下载，不会进入 aria2——此类签名 URL 在浏览器请求时即被消耗，或 CDN 会按请求特征返回不同内容（实测 aria2 拿到过损坏文件）。如确需转发，可在设置页关闭「跳过签名 URL 下载」
